...
DICTAPI void DictEng_SetLookupCancel(DictEng * pMe, boolean bCancel);
DICTAPI void DictEng_FreeWordInfo(DictEng * pMe, WordInfo * pWordInfo);
DICTAPI void DictEng_FreeSearchInfo(DictEng * pMe, SearchInfo * pSearchInfo);
DICTAPI boolean DictEng_New(DictEng ** ppiDictEng);
DICTAPI boolean DictEng_LoadDicts(DictEng * pMe, char ** pPaths, char ** pNames,
		int * pTypes, int count);
DICTAPI boolean DictEng_Lookup(DictEng * pMe, char * sWord, WordInfo * pWordInfo,
		int type);
DICTAPI boolean DictEng_LookupData(DictEng * pMe, const char *sWord,
		SearchInfo * pSearchInfo, PFN_PROGRESS pfn);
DICTAPI int DictEng_LookupWithRule(DictEng * pMe, const char *word, char *** pWordsList,
		PFN_PROGRESS pfn);
DICTAPI int DictEng_LookupWithFuzzy(DictEng * pMe, const char *sWord,
		char *** pWordsList, PFN_PROGRESS pfn);
DICTAPI int DictEng_ListWords(DictEng * pMe, char * sWord, char *** pWordsList);
DICTAPI int DictEng_DictCount(DictEng * pMe);
DICTAPI void DictEng_Release(DictEng * pMe);
